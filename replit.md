...
- **Microwave Leakage Testing**: Service card available for microwave oven radiation leakage testing (AS/NZS 60335.2.25 compliance standard for both Australia and New Zealand). On the item selection page, brands are displayed as clickable cards (grid layout) instead of regular items - 16 popular brands available (Samsung, LG, Panasonic, Sharp, Whirlpool, GE, Kenmore, Bosch, Electrolux, Hitachi, Toshiba, Siemens, Miele, KitchenAid, Frigidaire, Other). Each brand acts as an item; clicking a brand card navigates to the test page with the format "{Brand} Microwave" (e.g., "Samsung Microwave"). Currently routes through electrical test workflow; full microwave-specific testing workflow (radiation readings, door seal/interlock checks) available as future enhancement.
