...
Refined analytics tracking meaningful business events with clear, human-readable names:
- **User Events**: `User Logged In`, `User Logged Out`, `User Created`
- **Job Events**: `Job Started` (client, location, service, country), `Job Completed` (totalItems, passed, failed, passRate)
- **Testing Events**: `Item Selected`, `Test Recorded` (service, item, result)
- **Reports**: `Report Previewed`, `Report Downloaded` (format, client, items)
- **Certificates**: `Certificate Created/Edited/Downloaded/Deleted` (client, services)
- **Admin**: `Custom Environment Created`, `Admin Action`
- **Pages**: `Page Viewed` (with readable names like "Job Setup", "Report Preview")
- **Errors**: `Error Occurred`, `Server Error`

Environment variables: `POSTHOG_API_KEY`, `POSTHOG_HOST` (backend), `VITE_POSTHOG_API_KEY`, `VITE_POSTHOG_HOST` (frontend).